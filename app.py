...
Keep response under 300 words."""
    ...
    def validate_14_outlier_detection(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for col in numeric_cols[:10]:
            try:
                ...
                # Use 1.5*IQR rule (standard) and also flag extreme outliers (3*IQR)
    ...
    def validate_15_data_integrity(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for col in numeric_cols:
            ...
            if nan_count > len(df) * 0.1:  # More than 10% missing
        ...
        # Check string columns for data quality
        ...
        for col in string_cols[:10]:
            ...
            # Check for suspicious uniform length (possible placeholder data)
            if df[col].notna().any():
    ...
    def validate_24_quality_scoring(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        # ========== COMPLETENESS SCORING (Weight: 25%) ==========
        total_cells = df.shape[0] * df.shape[1]
        null_cells = df.isnull().sum().sum()
        null_percentage = (null_cells / total_cells * 100) if total_cells > 0 else 0
        
        # Strict penalties for missing data
        if null_percentage > 0:
            if null_percentage < 1:
                completeness_penalty = null_percentage * 5  # 0.1% nulls = 0.5 point penalty
            elif null_percentage < 5:
                completeness_penalty = 5 + (null_percentage - 1) * 8  # Escalating penalty
            else:
                completeness_penalty = 37 + (null_percentage - 5) * 2  # Severe penalty
        ...
        completeness = 100 - (null_percentage * 10)  # Heavy penalty
        scores['completeness'] = max(0, float(completeness))
        
        # ========== UNIQUENESS SCORING (Weight: 20%) ==========
        total_rows = len(df)
        duplicate_rows = df.duplicated().sum()
        duplicate_percentage = (duplicate_rows / total_rows * 100) if total_rows > 0 else 0
        
        # Strict penalties for duplicates
        if duplicate_rows > 0:
        ...
        for col in df.columns:
            if df[col].dtype == 'object':
                ...
                try:
                    ...
                    if 0.7 < conversion_rate < 0.95:  # Mixed types detected
    ...
    def run_selected_validations(self, df: pd.DataFrame, selected_validations: List[str]) -> Dict[str, Any]:
        ...
        for idx, validation_name in enumerate(selected_validations):
            status_text.text(f"Running: {validation_name}...")
        ...
        try:
            ...
            with st.spinner("🔄 Loading data..."):
                ...
                if uploaded_file.name.endswith('.csv'):
                    df = file_handler.read_csv(uploaded_file, encoding)
                elif uploaded_file.name.endswith(('.xlsx', '.xls')):
                    df = file_handler.read_excel(uploaded_file)
                elif uploaded_file.name.endswith('.json'):
                    df = file_handler.read_json(uploaded_file, encoding)
                elif uploaded_file.name.endswith('.txt'):
                    df = file_handler.read_txt(uploaded_file, encoding)
                else:
            ...
            if st.button("🚀 Run Validation", type="primary", use_container_width=True):
                ...
                try:
                    ...
                    results = validator.run_selected_validations(df, selected_validations)
                    
                    # Display Results with Provider Badge
                    ...
                    # Show penalty breakdown if quality score is not perfect
                    ...
                    if recommendations:
                        ...
                        for rec in recommendations[:15]:  # Show top 15
                            st.markdown(rec)
                    else:
...
AI Model: {model_name}
Quality Score: {quality_score:.2f}/100 ({quality_level})
...
"""
        ...
        with tab4:
            ...
            **Interpreting Results:**
            - 🟢 95-100: Perfect - Production-ready, zero critical issues
            - 🟢 90-94: Excellent - High quality, minimal issues
            - 🟡 80-89: Good - Acceptable with minor improvements needed
            - 🟠 70-79: Fair - Moderate issues requiring attention
            - 🔴 60-69: Poor - Significant issues, not production-ready
            - 🔴 <60: Critical - Major quality problems detected
            """)
